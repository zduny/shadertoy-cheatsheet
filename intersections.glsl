...
bool rayIntersectsSphere(in Ray ray, in Sphere sphere, out float t0, out float t1) {
...
bool rayIntersectsTriangle(in Ray ray, in vec3 v0, in vec3 v1, in vec3 v2, out float t) {
...
bool AABBintersectsAABB(in AABB a, in AABB b) {
	return all(lessThanEqual(a.min, b.max)) && all(greaterThanEqual(a.max, b.min));
}

bool sphereIntersectsAABB(in Sphere sphere, in AABB aabb, out vec3 intersection) {
